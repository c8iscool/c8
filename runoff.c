...
void tabulate(void)
{
    for (int i = 0; i < voter_count; i++)
    {
        ...
        else if ((candidates[preferences[i][0]].eliminated) == 1)
        {
            candidates[preferences[i][1]].votes++;
        }

        else if ((candidates[preferences[i][1]].eliminated) == 1)
        {
...
bool print_winner(void)
{
    for (int i = 0; i < candidate_count; i++)
    {
        int majority = round(0.5 * voter_count);
